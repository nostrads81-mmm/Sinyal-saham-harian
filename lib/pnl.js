...
// All-time realized P&L (rupiah) across every closed journal entry - used to
...
// same as computeNetPnl's own estimate-only case.
export function sumRealizedPnl(journalEntries, settings) {
  return journalEntries
    .filter((e) => e.status === 'CLOSE-PROFIT' || e.status === 'CLOSE-LOSS')
    .reduce((sum, e) => {
