...
describe('parseWatchlistRows', () => {
  ...
  test('DAY TRADE entry is the top of the buy range, never the last price', () => {
    const rows = grid([
      watchlistRow({ date: '03-08-2026', stock: 'INRANGE', buyRange: '100-110', last: '105', sl: '95 (-5%)', tp1: '120 (10%)', status: 'OPEN', keterangan: 'DAY TRADE' }),
      watchlistRow({ date: '03-08-2026', stock: 'ABOVE', buyRange: '100-110', last: '150', sl: '95 (-5%)', tp1: '120 (10%)', status: 'OPEN', keterangan: 'DAY TRADE' }),
    ]);
    const parsed = parseWatchlistRows(rows, { now: TODAY });
    const inRange = parsed.find((p) => p.stock === 'INRANGE');
    const above = parsed.find((p) => p.stock === 'ABOVE');
    expect(inRange.entry).toBe(110);
    expect(above.entry).toBe(110);
  });

  test('SWING TRADE entry is the midpoint of the buy range, never the last price', () => {
    const rows = grid([
      watchlistRow({ date: '03-08-2026', stock: 'INRANGE', buyRange: '100-110', last: '105', sl: '95 (-5%)', tp1: '120 (10%)', status: 'OPEN', keterangan: 'SWING TRADE' }),
  ...
  test('SWING TRADE rounds a fractional midpoint up, not down', () => {
    const rows = grid([
      watchlistRow({ date: '03-08-2026', stock: 'ODD', buyRange: '151-152', last: '151', sl: '145 (-4%)', tp1: '160 (5%)', status: 'OPEN', keterangan: 'SWING TRADE' }),
    ]);
    const parsed = parseWatchlistRows(rows, { now: TODAY });
  ...
  test('computes score as TP1% divided by SL%', () => {
    const rows = grid([
      // entry = 105 (midpoint of range, SWING TRADE), SL 100 -> 4.76%, TP1 120 -> 14.29% away -> score = 3
      watchlistRow({ date: '03-08-2026', stock: 'SCORED', buyRange: '100-110', sl: '100', tp1: '120', status: 'OPEN', keterangan: 'SWING TRADE' }),
    ]);
    const [signal] = parseWatchlistRows(rows, { now: TODAY });
...
describe('buildWaSignal', () => {
  test('defaults to DAY TRADE and uses the top of the buy range as entry', () => {
  ...
  test('SWING TRADE uses the midpoint of the buy range as entry, same rule as sheet signals', () => {
  ...
  test('SWING TRADE rounds a fractional midpoint up, not down', () => {
    const signal = buildWaSignal({ stock: 'ODD', tradeType: 'SWING TRADE', buyLow: 151, buyHigh: 152, sl: 145, tp1: 160 });
