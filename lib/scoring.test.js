...
describe('parseWatchlistRows', () => {
  ...
  test('keeps HOLD signals even after TP1+ has been hit (only the sheet removing a row drops it)', () => {
    const rows = grid([
      watchlistRow({
    ...
    const parsed = parseWatchlistRows(rows, { now: TODAY });
    expect(parsed.map((p) => p.stock).sort()).toEqual(['ALREADYTP', 'STILLGOING']);
    expect(parsed.find((p) => p.stock === 'ALREADYTP').highestTpReached).toBe(1);
  });
