...
describe('parseWatchlistRows', () => {
  ...
  test('entry is always the top of the buy range, never the last price', () => {
    const rows = grid([
      watchlistRow({ date: '03-08-2026', stock: 'INRANGE', buyRange: '100-110', last: '105', sl: '95 (-5%)', tp1: '120 (10%)', status: 'OPEN' }),
      watchlistRow({ date: '03-08-2026', stock: 'ABOVE', buyRange: '100-110', last: '150', sl: '95 (-5%)', tp1: '120 (10%)', status: 'OPEN' }),
    ]);
    const parsed = parseWatchlistRows(rows, { now: TODAY });
  ...
  test('drops a signal once TP1+ has been hit AND a trailing stop (TS) is set', () => {
    const rows = grid([
      watchlistRow({
        date: '01-08-2026', stock: 'TRAILING', buyRange: '100-110', sl: '95 (-5%)', tp1: '120 (10%)',
        status: 'RUNNING', detailStatus: 'HIT TP 1, HOLD & SET TS DI BUY PRICE',
      }),
      watchlistRow({
        date: '01-08-2026', stock: 'HOLDNOTS', buyRange: '100-110', sl: '95 (-5%)', tp1: '120 (10%)',
        status: 'RUNNING', detailStatus: 'HIT TP 1, HOLD',
      }),
      watchlistRow({
    ...
    const parsed = parseWatchlistRows(rows, { now: TODAY });
    expect(parsed.map((p) => p.stock).sort()).toEqual(['HOLDNOTS', 'STILLGOING']);
  });

  test('computes score as TP1% divided by SL%', () => {
    const rows = grid([
      // entry = 110 (top of range), SL 100 -> 9.09%, TP1 120 -> 9.09% away -> score ~1
      watchlistRow({ date: '03-08-2026', stock: 'SCORED', buyRange: '100-110', sl: '100', tp1: '120', status: 'OPEN' }),
    ]);
    const [signal] = parseWatchlistRows(rows, { now: TODAY });
    expect(signal.score).toBeCloseTo(1, 2);
  });
...
describe('buildWaSignal', () => {
  test('uses the top of the buy range as entry, same rule as sheet signals', () => {
