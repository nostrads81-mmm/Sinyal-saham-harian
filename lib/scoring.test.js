...
describe('rankSignals', () => {
  ...
  test('caps a position to the per-stock MM ceiling and names MM as the binding limit', () => {
    // MM 2% of a 50jt modal = 1jt ceiling, well under the ~8.33jt the risk
    // formula allows and the 50jt per-slot budget.
    const ranked = rankSignals([signal('MMCAP', { entry: 1000, sl: 970, mmPercent: 2 })], {
      capital: 50_000_000, riskPercent: 0.005, remainingCapital: 50_000_000,
      maxSlots: 6, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].willSkip).toBe(false);
    expect(ranked[0].position.rupiah).toBe(1_000_000);
    expect(ranked[0].position.lembar).toBe(1000);
    expect(ranked[0].adjusted).toBe(true);
    expect(ranked[0].capReason).toBe('mm');
  });

  test('an MM ceiling looser than the risk formula changes nothing', () => {
    const options = {
      capital: 50_000_000, riskPercent: 0.005, remainingCapital: 50_000_000,
      maxSlots: 6, occupiedSlots: 0, journaledStocks: new Set(),
    };
    const withoutMm = rankSignals([signal('PLAIN', { entry: 1000, sl: 970 })], options)[0];
    const looseMm = rankSignals([signal('PLAIN', { entry: 1000, sl: 970, mmPercent: 50 })], options)[0];

    expect(looseMm.position).toEqual(withoutMm.position);
    expect(looseMm.adjusted).toBe(false);
    expect(looseMm.capReason).toBe('risiko');
  });

  test('MM 0 or missing means no MM ceiling at all', () => {
    const options = {
      capital: 50_000_000, riskPercent: 0.005, remainingCapital: 50_000_000,
      maxSlots: 6, occupiedSlots: 0, journaledStocks: new Set(),
    };
    const zeroMm = rankSignals([signal('ZERO', { entry: 1000, sl: 970, mmPercent: 0 })], options)[0];
    const noMm = rankSignals([signal('NONE', { entry: 1000, sl: 970 })], options)[0];

    expect(zeroMm.position).toEqual(noMm.position);
    expect(zeroMm.adjusted).toBe(false);
    expect(zeroMm.capReason).toBe('risiko');
  });

  test('flags batas-mm when the MM ceiling cannot cover even 1 lot', () => {
    // MM 0.05% of 50jt = 25rb; one lot at 1000 costs 100rb. The modal is fine
    // here, so blaming "modal habis" would be wrong.
    const ranked = rankSignals([signal('TINYMM', { entry: 1000, sl: 970, mmPercent: 0.05 })], {
      capital: 50_000_000, riskPercent: 0.005, remainingCapital: 50_000_000,
      maxSlots: 6, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].willSkip).toBe(true);
    expect(ranked[0].skipReason).toBe('batas-mm');
    expect(ranked[0].capReason).toBe('mm');
  });

  test('the per-slot budget still wins when it is the smallest of the three caps', () => {
    // 5jt budget vs the MM 50% ceiling (25jt) vs ~18.5jt from the risk formula.
    const ranked = rankSignals([signal('BIG', { entry: 444, sl: 438, mmPercent: 50 })], {
      capital: 50_000_000, riskPercent: 0.005, remainingCapital: 5_000_000,
      maxSlots: 6, occupiedSlots: 0, journaledStocks: new Set(),
    });
