...
describe('rankSignals', () => {
  function signal(stock, { entry, sl, tp1, score, ageDays = 0, isOpen = true, isRunning = false } = {}) {
    const e = entry ?? 100;
    const s = sl ?? 95;
    const t = tp1 ?? 110;
    return {
      stock, entry: e, sl: s, tp1: t, tp2: null, tp3: null, ageDays,
  ...
  test('caps every position to the even share of the modal when no explicit cap is set', () => {
    // 350jt / 10 slot = 35jt ceiling; a 2% SL would otherwise be sized at 87,5jt.
    const ranked = rankSignals([signal('TIGHT', { entry: 1000, sl: 980 })], {
      capital: 350_000_000, riskPercent: 0.005, remainingCapital: 350_000_000,
      maxSlots: 10, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].position.rupiah).toBe(35_000_000);
    expect(ranked[0].position.lembar).toBe(35_000);
    expect(ranked[0].adjusted).toBe(true);
    expect(ranked[0].capReason).toBe('maks-saham');
    expect(ranked[0].capRupiah).toBe(35_000_000);
  });

  test('that ceiling does not grow just because few candidates showed up', () => {
    // Three candidates: the per-candidate budget alone would hand each of them
    // 116jt. The per-stock ceiling is what keeps it at 35jt.
    const signals = [
      signal('A', { entry: 1000, sl: 980 }), signal('B', { entry: 1000, sl: 980 }), signal('C', { entry: 1000, sl: 980 }),
    ];
    const ranked = rankSignals(signals, {
      capital: 350_000_000, riskPercent: 0.005, remainingCapital: 350_000_000,
      maxSlots: 10, occupiedSlots: 0, journaledStocks: new Set(),
    ...
    expect(ranked.map((r) => r.position.rupiah)).toEqual([35_000_000, 35_000_000, 35_000_000]);
    expect(ranked.every((r) => r.capReason === 'maks-saham')).toBe(true);
  });

  test('an explicit max-per-stock amount beats the automatic share', () => {
    const ranked = rankSignals([signal('TIGHT', { entry: 1000, sl: 980 })], {
      capital: 350_000_000, riskPercent: 0.005, remainingCapital: 350_000_000,
      maxSlots: 10, occupiedSlots: 0, journaledStocks: new Set(),
      maxPerStock: 20_000_000,
    });

    expect(ranked[0].position.rupiah).toBe(20_000_000);
    expect(ranked[0].capReason).toBe('maks-saham');
  });

  test('a wide-SL signal still gets its risk-based size, below the ceiling', () => {
    // 10% SL: 1,75jt / 0,10 = 17,5jt, under the 35jt ceiling - nothing is clipped,
    // so the 0,5% risk rule stands untouched.
...
describe('parseWaMessageText', () => {
  test('parses a standard single-signal WA message with "SL :" style', () => {
    ...
    expect(signal).toMatchObject({
      ...
      sl: 4400, tp1: 4700, tp2: 4800, confidence: 'high',
    });
    // The MM line is still in the message but deliberately not extracted.
    expect(signal.mmPercent).toBeUndefined();
  });
  ...
  test('parses "SL IF CLOSE <" style and Indonesian thousands separators', () => {
    ...
    const [signal] = parseWaMessageText(text);
    expect(signal).toMatchObject({
      stock: 'MEDC', tradeType: 'SWING TRADE', buyLow: 1200, buyHigh: 1250, sl: 1150, tp1: 1400,
    });
