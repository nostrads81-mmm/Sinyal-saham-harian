...
describe('buildWaSignal', () => {
  test('uses the midpoint of the buy range as entry, same rule as sheet signals', () => {
    const signal = buildWaSignal({ stock: 'ppre', buyLow: 101, buyHigh: 103, sl: 100, tp1: 110, tp2: 115 });
  ...
  test('carries a source tag through when the announcement was recognized as one (e.g. "koko_saham")', () => {
    const signal = buildWaSignal({ stock: 'PTRO', buyLow: 4925, buyHigh: 5075, sl: 4750, tp1: 5175, tag: 'koko_saham' });
    expect(signal.tag).toBe('koko_saham');
  });

  test('tag defaults to null when the announcement has no known source', () => {
    const signal = buildWaSignal({ stock: 'PPRE', buyLow: 101, buyHigh: 103, sl: 100, tp1: 110 });
    expect(signal.tag).toBeNull();
  });
...
describe('rankSignals', () => {
  function signal(stock, {
    entry, sl, tp1, score, ageDays = 0, isOpen = true, isRunning = false, mmPercent = null,
  } = {}) {
    const e = entry ?? 100;
    const s = sl ?? 95;
    const t = tp1 ?? 110;
  ...
  test('the per-slot budget still wins when it is the smallest of the three caps', () => {
    // 5jt budget vs ~18.5jt from the risk formula vs the 8,33jt modal share.
    const ranked = rankSignals([signal('BIG', { entry: 444, sl: 438 })], {
      capital: 50_000_000, riskPercent: 0.005, remainingCapital: 5_000_000,
      maxSlots: 6, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].willSkip).toBe(false);
    expect(ranked[0].capReason).toBe('jatah');
    expect(ranked[0].adjusted).toBe(true);
    expect(ranked[0].position.rupiah).toBeLessThanOrEqual(5_000_000);
  });
  ...
  test('a signal with its own MM% caps buy at capital x MM%, not an even per-slot split', () => {
    // MM 10%, capital 350jt -> max buy 35jt, even though there's only 1
    // candidate for 10 free slots (which would otherwise get the WHOLE
    // remaining budget under the plain per-slot split).
    const ranked = rankSignals([signal('MMCAP', { entry: 1000, sl: 900, mmPercent: 10 })], {
      capital: 350_000_000, riskPercent: 0.5, remainingCapital: 350_000_000,
      maxSlots: 10, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].willSkip).toBe(false);
    expect(ranked[0].capReason).toBe('mm');
    expect(ranked[0].capRupiah).toBe(35_000_000);
    expect(ranked[0].position.rupiah).toBeLessThanOrEqual(35_000_000);
  });

  test('MM% cap never exceeds actual remaining capital, even if capital x MM% is bigger', () => {
    // MM 50%, capital 350jt -> "max buy" 175jt, but only 2jt is actually left.
    const ranked = rankSignals([signal('MMCAP2', { entry: 100, sl: 90, mmPercent: 50 })], {
      capital: 350_000_000, riskPercent: 0.5, remainingCapital: 2_000_000,
      maxSlots: 10, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].willSkip).toBe(false);
    expect(ranked[0].capReason).toBe('sisa-modal');
    expect(ranked[0].position.rupiah).toBeLessThanOrEqual(2_000_000);
  });

  test('the same wide-budget scenario is not MM-capped when the signal has no mmPercent', () => {
    // Same shape as "a signal with its own MM% caps buy..." above, but no MM -
    // falls back to the per-slot split, which with only 1 candidate for 10
    // free slots hands it the WHOLE remaining budget instead of an MM% cap.
    const ranked = rankSignals([signal('NOMM', { entry: 1000, sl: 900, mmPercent: null })], {
      capital: 350_000_000, riskPercent: 0.5, remainingCapital: 350_000_000,
      maxSlots: 10, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].capReason).not.toBe('mm');
  });

  test('a wide-SL signal still gets its risk-based size, below the per-slot budget', () => {
    // 10% SL: 1,75jt / 0,10 = 17,5jt, well under the 350jt per-slot budget -
    // nothing is clipped, so the 0,5% risk rule stands untouched.
    const ranked = rankSignals([signal('WIDE', { entry: 1000, sl: 900 })], {
      capital: 350_000_000, riskPercent: 0.005, remainingCapital: 350_000_000,
      maxSlots: 10, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].position.rupiah).toBe(17_500_000);
    expect(ranked[0].adjusted).toBe(false);
    expect(ranked[0].capReason).toBe('risiko');
  });

  test('zero slot capacity means no candidate can be bought at all', () => {
    // Without slots to divide by there is also nowhere to put a position - the
    // per-stock ceiling cannot save a signal that has no slot to begin with.
    const ranked = rankSignals([signal('NOSLOTS', { entry: 1000, sl: 980 })], {
      capital: 50_000_000, riskPercent: 0.005, remainingCapital: 50_000_000,
      maxSlots: 0, occupiedSlots: 0, journaledStocks: new Set(),
    });

    expect(ranked[0].willSkip).toBe(true);
    expect(ranked[0].skipReason).toBe('slot-penuh');
  });
});
...
describe('parseWaMessageText', () => {
  ...
  test('parses the "MONEY MANAGEMENT : MAX BUY X% DARI MODAL" MM style', () => {
    const text = [
      'SWING TRADE - BUY HOPE : 260-282',
      'SL : 252 (-10.6%)',
      'TP 1 : 314 (11.34%)',
      'TP 2 : 344 (21.98%)',
      'TP 3 : 380 (34.75%)',
      'MONEY MANAGEMENT : MAX BUY 6% DARI MODAL',
      'RISK RATIO : 0.5% DARI MODAL',
      'RISK REWARD - 1:3.27',
    ].join('\n');
    const [signal] = parseWaMessageText(text);
    expect(signal).toMatchObject({
      stock: 'HOPE', tradeType: 'SWING TRADE', buyLow: 260, buyHigh: 282,
      sl: 252, tp1: 314, tp2: 344, mmPercent: 6, confidence: 'high',
    });
  });
  ...
  test('parses the narrative "maks buy / SL / TP" style with no trade-type label or colons', () => {
    const text = 'INET offer bagus, retest demand di daily. maks buy 352. SL 340. TP 370.';
    const [signal] = parseWaMessageText(text);
    expect(signal).toMatchObject({
      stock: 'INET', tradeType: 'DAY TRADE', buyLow: 352, buyHigh: 352, sl: 340, tp1: 370, confidence: 'low',
    });
    expect(signal.tp2).toBeUndefined();
  });

  test('narrative style also works with just "buy" (no "maks") and a lowercase stock code', () => {
    const text = 'bbri lanjut uptrend, buy 4500, SL 4400, TP 4700';
    const [signal] = parseWaMessageText(text);
    expect(signal).toMatchObject({ stock: 'BBRI', buyLow: 4500, sl: 4400, tp1: 4700 });
  });

  test('narrative style returns nothing when a required number is missing', () => {
    const text = 'INET offer bagus, retest demand di daily. maks buy 352.';
    expect(parseWaMessageText(text)).toEqual([]);
  });

  test('parses the "Entry:"-labeled style with the stock code alone on the first line', () => {
    const text = [
      'PTRO',
      'Entry : 4925-5075',
      'TP 1 : 5175',
    ...
    const [signal] = parseWaMessageText(text);
    expect(signal).toMatchObject({
      stock: 'PTRO', tradeType: 'DAY TRADE', buyLow: 4925, buyHigh: 5075, sl: 4750, tp1: 5175, tp2: 5375,
      confidence: 'high', tag: 'koko_saham',
    });
    // TP3 isn't kept - WA_Signals has no TP3 column, same as every other format.
    expect(signal.tp3).toBeUndefined();
  });

  test('"Entry:"-labeled style also works with just TP1 (no TP2/TP3)', () => {
    const text = 'BBRI\nEntry : 4500-4550\nTP 1 : 4700\nSL : 4400';
    const [signal] = parseWaMessageText(text);
    expect(signal).toMatchObject({ stock: 'BBRI', buyLow: 4500, buyHigh: 4550, sl: 4400, tp1: 4700 });
    expect(signal.tp2).toBeUndefined();
  });

  test('"Entry:"-labeled style returns nothing when a required field is missing', () => {
    const text = 'PTRO\nEntry : 4925-5075\nHigh risk';
    expect(parseWaMessageText(text)).toEqual([]);
  });
});
