...
// pasted.
const WA_SIGNALS_RANGE = 'WA_Signals!A2:J1000';
...
function signalToWaRow(s) {
  return [
    ...
    s.tp2 ?? '', s.mmPercent ?? '', s.capturedAt, s.tag ?? '',
  ];
}
...
// WA_Signals is a whole-tab replace (clear + rewrite), not an append, so every
// mutation is a read-modify-write. Without serialization two overlapping
// mutations (e.g. a save tap right after a background fetch effect, or two
// quick "hapus" taps) interleave: A reads, B reads, A clears+writes, B
// clears+writes - and B's snapshot, taken before A's write, silently drops
// whatever A just added. Chaining each mutation for a given sheetId behind the
// previous one makes the read→merge→write span atomic per session.
...
async function writeWaSignalRows(token, sheetId, list) {
  // Write the fresh rows first, then clear only the tail rows the new list no
  // longer covers. The old clear-everything-then-write order left an empty
  // sheet (and, on any mid-write failure, lost data) between the two calls;
  // this way the sheet always holds a valid list, just possibly with a few
  // stale trailing rows if the request is interrupted.
  if (list.length > 0) {
    await setValues(sheetId, 'WA_Signals!A2', list.map(signalToWaRow), token);
  }
  const clearFrom = list.length + 2;
  await clearValues(sheetId, `WA_Signals!A${clearFrom}:J1000`, token);
}
...
export function addWaSignalRows(token, sheetId, signals) {
...
export function removeWaSignalRow(token, sheetId, stock) {
  return withWaSignalLock(sheetId, async () => {
...
export function pruneStaleWaSignalRows(token, sheetId, staleStocks) {
  if (staleStocks.length === 0) return Promise.resolve();
  return withWaSignalLock(sheetId, async () => {
