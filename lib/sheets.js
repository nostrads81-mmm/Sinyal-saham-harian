...
// Also migrates an existing-but-older Settings row that predates the fee
// columns, by appending just the missing headers/defaults after whatever is
// already there (so the user's own Modal/Risiko/Slot values aren't touched).
export async function ensureSheetsInitialized(token) {
  for (const [tab, headers] of Object.entries(SHEET_HEADERS)) {
    ...
    if (existing.length === 0 || existing[0].length === 0) {
      ...
      await setValues(APP_DATA_SHEET_ID, `Settings!${startCol}1`, [headers.slice(currentCount)], token);
      await setValues(APP_DATA_SHEET_ID, `Settings!${startCol}2`, [DEFAULT_SETTINGS_VALUES.slice(currentCount)], token);
    }
  }
}

export async function getSettings(token) {
  const rows = await getValues(APP_DATA_SHEET_ID, 'Settings!A2:G2', token);
...
export async function updateSettings(token, { capital, maxSlots }) {
  await setValues(APP_DATA_SHEET_ID, 'Settings!A2', [[capital]], token);
  await setValues(APP_DATA_SHEET_ID, 'Settings!C2', [[maxSlots]], token);
}
...
async function getActiveJournalRows(token) {
  const rows = await getValues(APP_DATA_SHEET_ID, 'DayTrade_Journal!A2:J500', token);
  return rows.filter((r) => ['RUNNING', 'OPEN'].includes((r[6] || '').toUpperCase()));
}

export async function getActiveJournalCount(token) {
  return (await getActiveJournalRows(token)).length;
}
...
export async function getJournaledStocks(token) {
  const rows = await getActiveJournalRows(token);
  return new Set(rows.map((r) => (r[1] || '').toUpperCase()));
}
...
export async function getInvestedCapital(token) {
  const rows = await getActiveJournalRows(token);
