...
const SHEET_HEADERS = {
  ...
  // MmPercent (money management %, e.g. "MM : 10%" in the WA message) briefly
  // went unused between 2026-09-21 and 2026-09-24 while position sizing used a
  // flat "Maks per Saham" ceiling instead - that ceiling was reverted in favor
  // of MM, so this column is populated again.
  WA_Signals: [
  ...
  // Ledger of manual capital deposits/withdrawals ("Tambah/Kurang Modal" in
  // Settings), added 2026-09-24. Jumlah is signed (positive = setor, negative
  // = tarik) - each entry also nudges Settings' Modal Awal by the same amount
  // at the time it's recorded, so this sheet is purely a history log, not the
  // source of truth for the current Modal Awal value.
  Modal_Histori: ['Tanggal', 'Jumlah', 'Keterangan'],
};
...
// Returns every deposit/withdrawal ever logged via "Tambah/Kurang Modal" in
// Settings, oldest first - purely a history log for display, see the
// Modal_Histori comment on SHEET_HEADERS above.
export async function getModalHistory(token, sheetId) {
  const rows = await getValues(sheetId, 'Modal_Histori!A2:C1000', token);
  return rows
    .map((r, i) => ({
      rowNumber: i + 2,
      tanggal: r[0] || '',
...
// Records one capital deposit/withdrawal and immediately folds it into
// Settings' Modal Awal (jumlah is signed - positive adds, negative
// withdraws) - the two writes aren't atomic, but a failure between them just
// leaves the ledger one entry ahead of Modal Awal, visible and correctable,
// rather than silently wrong.
export async function addModalTransaction(token, sheetId, { tanggal, jumlah, keterangan, currentCapital }) {
  await appendValues(sheetId, 'Modal_Histori!A:C', [[tanggal, jumlah, keterangan || '']], token);
  const newCapital = currentCapital + jumlah;
  await setValues(sheetId, 'Settings!A2', [[newCapital]], token);
  return newCapital;
}
