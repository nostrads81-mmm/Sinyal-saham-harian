...
async function fetchWithAuthRetry(url, token, options, apiLabel, purpose = 'main') {
  ...
  if (res.status === 401) {
    const freshToken = await refreshAccessToken(purpose);
...
async function sheetsFetch(path, token, options = {}, purpose = 'main') {
  return fetchWithAuthRetry(`${BASE}${path}`, token, options, 'Sheets', purpose);
}
...
// `purpose` selects which of the two independent login slots (see lib/auth.js)
// to silently refresh on a 401 - defaults to 'main' (Sinyal/Rekapan's own
// account); Watchlist passes 'watchlist' when reading the shared source
// sheet under a different account.
export async function getValues(spreadsheetId, range, token, purpose = 'main') {
...
// Points at a spreadsheet owned by the app's own "watchlist" account (see
// lib/auth.js purpose='watchlist'), auto-synced every 15 min from the real
// community source sheet via a Google Apps Script trigger on that copy -
// the original is owned by a third party who only granted Viewer access,
// so it couldn't be re-shared to other users directly.
