...
export function parseWatchlistRows(rows, { tradeType = ['DAY TRADE', 'SWING TRADE'], now = new Date() } = {}) {
  ...
  for (const row of dataRows) {
    ...
    // buy range as a balanced estimate, rounded UP when it lands on a
    // fraction (e.g. 151.5 -> 152) since IDX prices are whole rupiah. We
...
// buy range, rounded up on a fraction.
