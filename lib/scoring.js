...
export function parseWatchlistRows(rows, { tradeType = ['DAY TRADE', 'SWING TRADE'], now = new Date() } = {}) {
  ...
  for (const row of dataRows) {
    ...
    // Drop once TP1+ has been hit AND a trailing stop (TS) has been set -
    // that combination means the position is just riding a trail, not a
    // fresh entry opportunity anymore. A plain HOLD (no TS yet) still shows.
    const hasTrailingStop = /\bTS\b/i.test(detailStatus);
    if (highestTpReached >= 1 && hasTrailingStop) continue;
...
// together. Same entry rule as the sheet-based signals: always the top of
// the buy range (maximizes fill probability - see the note on the sheet
// version above), not a midpoint guess.
