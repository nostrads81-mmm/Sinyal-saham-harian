...
export function parseWatchlistRows(rows, { tradeType = ['DAY TRADE', 'SWING TRADE'], now = new Date() } = {}) {
  ...
  for (const row of dataRows) {
    ...
    // (Reversed 2026-08-05: signals that already hit TP1+ but are still HOLD
    // (trailing stop, position still open) must stay visible. Rows only drop
    // out once the sheet owner actually removes them.)
