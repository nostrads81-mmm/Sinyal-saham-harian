...
export function buildWaSignal({
  stock, tradeType, buyLow, buyHigh, sl, tp1, tp2, capturedAt, entryMode = 'mid', tpMode = 'mid',
...
// - The "Maks per Saham" ceiling from Pengaturan (a fixed rupiah amount, or the
//   even share of the modal when left at 0) caps the size. The buy value is the
//   smallest of the risk formula, that ceiling, and the even per-slot budget.
//   `capReason` records which one actually decided it, so the UI can say the
//   real reason instead of always blaming the modal.
...
export function rankSignals(parsedRows, {
  ...
  // Ceiling per stock, from Pengaturan. An explicit rupiah amount wins; left at
  // 0 it falls back to the even share of the modal (modal / slot capacity).
  // Unlike perSlotBudget this does not depend on how many candidates exist
  // today - otherwise a day with only 3 signals would let each of them take a
  // third of the modal, which is what "maks per saham" is meant to prevent.
  const perStockCap = maxPerStock > 0
    ? maxPerStock
    : (maxSlots > 0 ? capital / maxSlots : Infinity);
  ...
  const rankedOpen = openRows.map((r, i) => {
    ...
    const caps = [
      ...
      { reason: 'maks-saham', rupiah: perStockCap },
      { reason: 'jatah', rupiah: perSlotBudget },
    ];
    // Compared on rounded rupiah: the risk formula and the even modal share can
    // land on the same number via different arithmetic (250rb / 3% vs 50jt / 6),
    // and floating-point noise in the last digits must not make the app claim
    // "dibatasi maks per saham" when both limits are really the same amount.
