...
// Deterministic parser for the standard WA-announcement text format (see
// WA_SIGNAL_PROMPT in pages/api/extract-screenshot.js for the exact shape
// this mirrors) - lets a copy-pasted message skip the Gemini round trip
// entirely when it already matches the well-known layout, instead of
// waiting a few seconds on an AI call for something a handful of regexes
// can read instantly and for free. Returns [] (not an error) when the text
// doesn't look like this format at all, so the caller can fall back to the
// AI extraction endpoint for anything looser/unexpected.
