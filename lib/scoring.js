...
export function rankSignals(parsedRows, {
  ...
  const perSlotBudget = availableSlots > 0 ? remainingCapital / availableSlots : 0;
  ...
  const passthroughJournaled = journaledOpenRows.map((r) => ({
    ...r, rank: null, willSkip: true, skipReason: 'sudah-terbeli', position: { rupiah: 0, lembar: 0 },
  }));
  const passthroughOthers = otherRows.map((r) => ({
    ...r, rank: null, willSkip: false, skipReason: null, position: null,
  }));

  return [...rankedOpen, ...passthroughJournaled, ...passthroughOthers];
}
