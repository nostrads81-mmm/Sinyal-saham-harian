...
// Score (risk/reward ratio) basis: the midpoint of TP1 and TP2 when TP2 is
// known, not TP1 alone - TP1 by itself is a conservative target, and
// averaging in TP2 gives a more representative reward figure for ranking
// signals against each other. Falls back to TP1 alone when there's no TP2.
// This does NOT change tp1Percent, the "TP1 xxx%" shown to the user and
// copied into the AI prompt - that still reflects TP1 exactly, only the
// score/ranking basis is different.
function computeScore(entry, sl, tp1, tp2) {
  const slPercent = Math.abs((entry - sl) / entry) * 100;
  // Rounded to a whole rupiah since it's shown to the user as "TP" on the
  // card (see tpMid below) - not just an internal ratio anymore.
  const tpMid = Math.round(tp2 != null ? (tp1 + tp2) / 2 : tp1);
  const scoreTpPercent = Math.abs((tpMid - entry) / entry) * 100;
...
export function parseWatchlistRows(rows, { tradeType = ['DAY TRADE', 'SWING TRADE'], now = new Date(), entryMode = 'mid' } = {}) {
  ...
  for (const row of dataRows) {
    ...
    const { slPercent, score, tpMid } = computeScore(entry, sl.price, tp1.price, tp2.price);
...
export function buildWaSignal({ stock, tradeType, buyLow, buyHigh, sl, tp1, tp2, mmPercent, capturedAt, entryMode = 'mid' }) {
  const resolvedTradeType = tradeType === 'SWING TRADE' ? 'SWING TRADE' : 'DAY TRADE';
  const entry = computeEntry(buyLow, buyHigh, entryMode);
  const { slPercent, score, tpMid } = computeScore(entry, sl, tp1, tp2);
