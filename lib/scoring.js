...
export function parseWatchlistRows(rows, { tradeType = ['DAY TRADE', 'SWING TRADE'], now = new Date() } = {}) {
  ...
  for (const row of dataRows) {
    ...
    // Single entry point rule (revised 2026-08-06): use the midpoint of the
    // buy range as a balanced estimate for both DAY TRADE and SWING TRADE,
    // rounded UP when it lands on a fraction (e.g. 151.5 -> 152) since IDX
    // prices are whole rupiah. The full range is kept on the signal too
    // (buyLow/buyHigh) so the UI can show it alongside the midpoint. We
    // still flag "wait" when price hasn't even reached the range yet.
    const entry = Math.ceil((buyRange.low + buyRange.high) / 2);
...
export function buildWaSignal({ stock, tradeType, buyLow, buyHigh, sl, tp1, tp2, mmPercent, capturedAt }) {
  ...
  const entry = Math.ceil((buyLow + buyHigh) / 2);
