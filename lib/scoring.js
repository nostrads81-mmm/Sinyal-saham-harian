...
// and there's no TP2 to look for.
...
export function parseWaMessageText(text) {
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    // Two admin styles for the same field: short "MM : 9%" and the fuller
    // "MONEY MANAGEMENT : MAX BUY 6% DARI MODAL" - both mean the MM% cap
    // used for position sizing in rankSignals below.
    const mmMatch = block.match(/MM\s*:\s*([\d.,]+)\s*%/i)
      || block.match(/MONEY MANAGEMENT\s*:\s*MAX BUY\s*([\d.,]+)\s*%/i);
...
// - MM (money management) rule, 2026-09-24: a signal carrying its own
//   mmPercent (from "MM : X%" or "MONEY MANAGEMENT : MAX BUY X% DARI MODAL"
//   in the WA message) caps its buy at capital x mmPercent instead of
//   splitting the remaining budget evenly across today's open slots - each
//   such signal sizes itself off the account's total capital, not a slice of
//   it. A signal with no MM value of its own falls back to the even
//   per-slot split. Either way the buy value never exceeds the actual
//   remaining capital. `capReason` records which cap actually decided it, so
//   the UI can say the real reason instead of always blaming the modal.
// - Only the top `availableSlots` candidates (by score) get a slot; the rest
//   are "slot-penuh". Within a granted slot, if even the binding cap can't
...
export function rankSignals(parsedRows, {
  ...
  openRows.sort((a, b) => {
    ...
    // Tie-breaker 2: fresher signal wins - an OPEN signal that's sat unfilled
  ...
  const rankedOpen = openRows.map((r, i) => {
    ...
    const caps = [
      ...
      r.mmPercent != null
        ? { reason: 'mm', rupiah: capital * (r.mmPercent / 100) }
        : { reason: 'jatah', rupiah: perSlotBudget },
      { reason: 'sisa-modal', rupiah: remainingCapital },
    ];
    // Compared on rounded rupiah: different caps can land on the same number
    // via different arithmetic, and floating-point noise in the last digits
    // must not make the app claim "dibatasi MM" when both limits are really
    // the same amount.
    const binding = caps.reduce(
      (smallest, c) => (Math.round(c.rupiah) < Math.round(smallest.rupiah) ? c : smallest)
    );
    ...
    return {
      ...
      capReason: binding.reason,
      // The ceiling that decided the size, in rupiah - the card names the real
      // reason ("maks 35 jt per saham") instead of guessing from the reason code.
      capRupiah: binding.rupiah,
    };
  });
