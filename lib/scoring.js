...
export function parseWatchlistRows(rows, { tradeType = ['DAY TRADE', 'SWING TRADE'], now = new Date() } = {}) {
  ...
  for (const row of dataRows) {
    ...
    // Entry point rule (revised 2026-08-06): DAY TRADE uses the TOP of the
    // buy range - a day trade needs the position filled fast, so we entry
    // right where the range tops out rather than waiting for a better
    // price. SWING TRADE keeps the midpoint as a balanced estimate, since
    // there's more time for price to come back toward the middle. Rounded
    // UP when it lands on a fraction (e.g. 151.5 -> 152) since IDX prices
    // are whole rupiah. We still flag "wait" when price hasn't even
    // reached the range yet.
    const entry = keterangan === 'DAY TRADE'
      ? Math.ceil(buyRange.high)
      : Math.ceil((buyRange.low + buyRange.high) / 2);
...
// together. Same entry rule as the sheet-based signals: DAY TRADE uses the
// top of the buy range, SWING TRADE uses the midpoint, rounded up on a
// fraction.
export function buildWaSignal({ stock, tradeType, buyLow, buyHigh, sl, tp1, tp2, mmPercent, capturedAt }) {
  ...
  const entry = resolvedTradeType === 'DAY TRADE'
    ? Math.ceil(buyHigh)
    : Math.ceil((buyLow + buyHigh) / 2);
