...
// Rank OPEN signals (actionable = fresh buy opportunity) and mark the rest
// as SKIP once slots run out. RUNNING signals are handled separately by
// sortRunningSignals() - they don't compete for slots, they're just tracked.
export function rankSignals(parsedRows, { openSlots }) {
  ...
  return actionable.map((r, i) => ({ ...r, rank: i + 1, willSkip: i >= openSlots }));
}
