...
export function parseWatchlistRows(rows, { tradeType = 'DAY TRADE', now = new Date() } = {}) {
  ...
  for (const row of dataRows) {
    ...
    // Drop entirely once ANY TP has been hit (2026-08-03: user wants signals
    // that already reached TP1+ out of the list, not just fully-done TP3).
    if (highestTpReached >= 1) continue;

    // Single entry point rule (revised 2026-08-01): always use the top of the
    // buy range. A snapshot "last price" entry looked more precise but often
    // never got touched again once the market moved (confirmed with PGEO -
    // recommended entry 1010 never re-traded after opening straight at 1020),
    // so the order silently never filled. The whole range is valid per the
    // signal itself, so picking the top trades a slightly worse SL:TP ratio
    // for a much higher chance the order actually executes. We still flag
    // "wait" when price hasn't even reached the range yet.
    const entry = buyRange.high;
...
// Attaches budget-based rank/position info to every OPEN signal (greedily
// allocating real remaining capital in score order) and passes everything
// else through untouched. Returns the FULL list, still mixed - grouping by
// publish date ("hari ini" vs "kemarin") happens separately in the page, not
// here, since that's a display concern independent of buy-ability.
// - A stock already sitting in the journal (bought) is skipped with its own
//   reason ("sudah-terbeli") instead of eating into the budget calc at all.
// - When the ideal risk-based position doesn't fully fit the budget left,
//   the lot size is scaled DOWN to whatever still fits (rather than skipped
//   outright) - only skipped once there's not even 1 lot (100 shares) worth
//   of budget remaining.
export function rankSignals(parsedRows, { capital, riskPercent, remainingCapital, journaledStocks = new Set() }) {
  const openRows = parsedRows.filter((r) => r.isOpen);
  ...
  let budget = remainingCapital;
  const rankedOpen = openRows.map((r, i) => {
    if (journaledStocks.has(r.stock.toUpperCase())) {
      return { ...r, rank: i + 1, willSkip: true, skipReason: 'sudah-terbeli', position: { rupiah: 0, lembar: 0 } };
    }

    const full = positionSize(r.entry, r.sl, capital, riskPercent);
    const cappedRupiah = Math.min(full.rupiah, budget);
    ...
    const rupiah = lembar * r.entry;
    budget -= rupiah;
    return {
      ...r, rank: i + 1, willSkip: false, skipReason: null,
      position: { rupiah, lembar }, adjusted: lembar < full.lembar,
    };
  });

  const passthroughOthers = otherRows.map((r) => ({
    ...r, rank: null, willSkip: false, skipReason: null, position: null,
  }));

  return [...rankedOpen, ...passthroughOthers];
}
