...
// Rank OPEN signals (actionable = fresh buy opportunity) and mark SKIP once
// the real remaining capital runs out - a simple position COUNT can't tell
// whether 6 slots means "6 small trades, tons of cash left over" or "6 huge
// trades, way over budget", so budgeting greedily against actual rupiah is
// what decides who gets skipped, not an arbitrary slot number.
export function rankSignals(parsedRows, { capital, riskPercent, remainingCapital }) {
  ...
  return actionable.map((r, i) => {
    const positionRupiah = positionSize(r.entry, r.sl, capital, riskPercent).rupiah;
    const willSkip = positionRupiah > budget;
    if (!willSkip) budget -= positionRupiah;
    return { ...r, rank: i + 1, willSkip, positionRupiah };
  });
}
