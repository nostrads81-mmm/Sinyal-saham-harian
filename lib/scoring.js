...
export function buildWaSignal({
  ...
  return {
    ...
    // "catat" from Watchlist carries the sheet's own tanggal as capturedAt,
    // so a signal recorded a few days after it first appeared shows its
    // real age instead of always reading "hari ini".
    ageDays: capturedAt ? (daysBetween(new Date(capturedAt), new Date()) || 0) : 0,
    entry,
    buyLow,
    buyHigh,
    estimatedEntry: true,
