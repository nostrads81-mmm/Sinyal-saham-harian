...
export function parseWatchlistRows(rows, { tradeType = 'DAY TRADE', now = new Date() } = {}) {
  ...
  for (const row of dataRows) {
    ...
    // Signal is considered done once TP3 has been hit - drop it entirely.
    if (highestTpReached >= 3) continue;
...
// Rank OPEN signals (actionable = fresh buy opportunity) against the real
// remaining capital, greedily allocating budget in score order.
...
// RUNNING signals: most-progressed (closest to TP3) first, then best score.
export function sortRunningSignals(parsedRows) {
  const running = parsedRows.filter((r) => r.isRunning);
  running.sort((a, b) => {
    if (b.highestTpReached !== a.highestTpReached) return b.highestTpReached - a.highestTpReached;
