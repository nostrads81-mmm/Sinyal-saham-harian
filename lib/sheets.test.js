...
describe('getSettings / updateSettings', () => {
  ...
  test('writes fee and materai cells (D2:G2) only when provided', async () => {
    const calls = captureFetch();

    await updateSettings('token', 'sheet123', {
      capital: 350_000_000, maxSlots: 10,
      buyFeePercent: 0.0015, sellFeePercent: 0.0025, materaiAmount: 10000, materaiThreshold: 10000000,
    });

    expect(calls.filter((c) => c.range.startsWith('Settings!')).map((c) => [c.range, c.body.values]))
      .toEqual([
        ['Settings!A2', [[350_000_000]]],
        ['Settings!C2', [[10]]],
        ['Settings!D2', [[0.0015]]],
        ['Settings!E2', [[0.0025]]],
        ['Settings!F2', [[10000]]],
        ['Settings!G2', [[10000000]]],
      ]);
  });
});

describe('Modal_Histori (Tambah/Kurang Modal ledger)', () => {
  const { getModalHistory, addModalTransaction } = require('./sheets');

  beforeEach(() => {
    jest.resetAllMocks();
  });

  test('addModalTransaction appends a signed row and folds it into Modal Awal', async () => {
    const calls = [];
    global.fetch = jest.fn((url, options = {}) => {
      calls.push({
        url: String(url),
    ...
    const newCapital = await addModalTransaction('token', 'sheet123', {
      tanggal: '24-09-2026', jumlah: -5_000_000, keterangan: 'Tarik buat kebutuhan', currentCapital: 50_000_000,
    });

    expect(newCapital).toBe(45_000_000);
    const append = calls.find((c) => c.url.includes(':append'));
    expect(append.body.values).toEqual([['24-09-2026', -5_000_000, 'Tarik buat kebutuhan']]);
    const capitalWrite = calls.find((c) => c.method === 'PUT' && c.url.includes('Settings!A2'));
    expect(capitalWrite.body.values).toEqual([[45_000_000]]);
  });

  test('getModalHistory reads every logged transaction, skipping blank rows', async () => {
    global.fetch = jest.fn(() => Promise.resolve({
      ok: true, status: 200,
      json: () => Promise.resolve({
        values: [
          ['24-09-2026', '5000000', 'Setor awal'],
          ['', '', ''],
        ],
      }),
    }));

    const history = await getModalHistory('token', 'sheet123');

    expect(history).toEqual([
      { rowNumber: 2, tanggal: '24-09-2026', jumlah: 5_000_000, keterangan: 'Setor awal' },
    ]);
  });
});
  ...
  test('an older sheet without DeletedAt gets just that header appended', async () => {
    ...
    global.fetch = jest.fn((url, options = {}) => {
      ...
      if (method === 'GET' && target.includes('fields=sheets.properties.title')) {
        // Modal_Histori already exists too (added at the same time as this test's
        // sheet was created) but with no header row yet - a separate, unrelated
        // write below, not part of the WA_Signals migration this test covers.
        const titles = ['DayTrade_Journal', 'Settings', 'WA_Signals', 'Modal_Histori'];
        return Promise.resolve({
          ok: true, status: 200,
          json: () => Promise.resolve({ sheets: titles.map((title) => ({ properties: { title } })) }),
        });
      ...
      if (method === 'GET' && target.includes('!A1:Z1')) {
        const tab = ['DayTrade_Journal', 'Settings', 'WA_Signals', 'Modal_Histori'].find((t) => target.includes(t));
        // WA_Signals at 10 columns = the shape every existing account still has,
        // written before the deletion marker existed. Modal_Histori at 0 = a
        // brand new, still-empty tab.
        const counts = { DayTrade_Journal: 12, Settings: 10, WA_Signals: 10, Modal_Histori: 0 };
        return Promise.resolve({
          ok: true, status: 200,
    ...
    // of real data above it are never touched. Modal_Histori's own header write
    // (a separate, brand-new tab) is a distinct, expected write alongside it.
    const waWrites = writes.filter((w) => w.target.includes('WA_Signals'));
    expect(waWrites).toHaveLength(1);
    expect(waWrites[0].target).toContain('WA_Signals!K1');
    expect(waWrites[0].values).toEqual([['DeletedAt']]);
  });
